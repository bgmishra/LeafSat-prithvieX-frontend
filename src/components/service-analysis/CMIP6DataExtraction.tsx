...
export function CMIP6DataExtraction() {
  const router = useRouter();
  const isDataExtraction = false;
  const isCmip6DataExtraction = true;
  const isExtractionWorkflow = true;
  const serviceCategoryName = "CMIP6 Data Extraction";
  ...
  const processingCharge = pricingEstimate ? pricingEstimate.finalPrice : 0;
...
//     let responseProcess: SubmitProcessingJobResponse | null = null;

// try {
//   responseProcess = await submitProcessingJob(
//     pendingProcessingPayload,
//     pricingEstimate.finalPrice
//   );
...
//     previous == null
//       ? previous
//       : Math.max(0, previous - pricingEstimate.finalPrice)
//   );
// } catch (caught) {
//   setPendingProcessingPayload(null);
//   setPricingEstimate(null);
...
// } finally {
//   if (responseProcess) {
//     router.push(`/orders/${responseProcess.jobId}`);
//   }
